...
def load(df):
    ...
    dim_customer.columns = ["customer_code", "country"]
    ...
    dim_date_ready = dim_date_ready[["date", "year", "month", "day", "weekday"]]
    dim_date_ready = dim_date_ready.drop_duplicates(subset=["date"]) 
    ...
    # Veri patlaması kontrolü 
    if len(final_fact) > len(df) + 100: 
